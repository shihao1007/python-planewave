...
def focused_planewave_spherical_coordinates(NA, NumSample, kd):
    ...
    for i in range(NumSample ** 2):
        ...
        Fp = Fp + P1.evaluate(X, Y, Z, Ei)

    return Fp
        
def two_layers(s1, s2, pw, X, Y, Z):
    ...
    if(theta_0 == 0):
        ...
        if(tir1 == True):
            ...
            r01s = (sin_theta_1 * cos_theta_0 - sin_theta_0 * cos_theta_1) / (sin_theta_1 * cos_theta_0 + sin_theta_0 * cos_theta_1)
            r01p = (sin_theta_1 * cos_theta_1 - sin_theta_0 * cos_theta_0) / (sin_theta_1 * cos_theta_1 + sin_theta_0 * cos_theta_0)
            t01s = 2 * sin_theta_1 * cos_theta_0 / (sin_theta_1 * cos_theta_0 + sin_theta_0 * cos_theta_1)
            t01p = 2 * sin_theta_1 * cos_theta_0 / (sin_theta_1 * cos_theta_1 + sin_theta_0 * cos_theta_0)
...
#            Er1s = Ei_s * t01s * d * ePlus * r12s * d / (1 - ePlus * r12s * d * r01s * d)         
            ...
            Ft1 = Wt1.evaluate(X, Y, Z)
            Fr1 = Wr1.evaluate(X, Y, Z)
            Ft2 = Wt2.evaluate(X, Y, Z)
            
            mask0 = np.zeros(Z.shape)
            mask1 = np.zeros(Z.shape)
...
F2s = two_layers(P1, P2, Ei, X, Y, Z)
#plot the field
fig = plt.figure()

plt.subplot(131)
plt.imshow(np.abs(F2s[0, :, :]))
plt.title('Absolute Value')

plt.subplot(132)
plt.imshow(np.real(F2s[0, :, :]))
plt.title('Real Part')

plt.subplot(133)
plt.imshow(np.imag(F2s[0, :, :]))
