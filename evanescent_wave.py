...
class boundary:
    ...
    def scatter(self, pw):
        ...
        if (sin_theta_t > 1):               #if sin theta t is bigger than 1
            ...
            cos_theta_t = 1j * np.sqrt(self.sin_theta_i**2 * n0**2 / n1**2 - 1)  #compute cos theta t for later use for complex TIR
        else:
        ...
        if (theta_i == 0):              #when the plane wave hits the interface head-on, s-components of the plane wave will be the same
            ...
            self.kt = pw.k * np.real(self.n1) / np.real(self.n0)            #transmitted k vector is degenerated by nr
            Er = pw.E * rp            #reflected field
            Et = pw.E * tp            #transmitted field
            phase_r = np.dot(self.p, pw.k - kr)  #compute the phase offset
            phase_t = np.dot(self.p, pw.k - self.kt)  #??? what is phase offset?
            ...
            if (self.tir == True):
                ...
                ininner = (self.sin_theta_i * self.n0 / self.n1 ) ** 2 - 1 + 0j           #value inside square root
                inner_s = self.n1 / (self.n0 * cos_theta_i) * np.sqrt(ininner)         #value before arctan
                rs = np.exp( -2 * 1j * np.arctan(inner_s))              #compute rs
                ts = 2 * n0 * cos_theta_i / (n0 * cos_theta_i + n1 * cos_theta_t)                                                 #set ts to 0 for now
                
                inner_p = self.n0 / (self.n1 * cos_theta_i) * np.sqrt(ininner)   #same thing for rp and tp
                rp = np.exp( -2 * 1j * np.arctan(inner_p))
                tp = 2 * n0 * cos_theta_i / (n0 * cos_theta_t + n1 * cos_theta_i)
                
                kr = ( yHat * self.sin_theta_i - zHat * cos_theta_i) * np.linalg.norm(pw.k) * np.real(self.n0)  #compute kr
                self.kt = ( yHat * sin_theta_t + zHat * cos_theta_t) * np.linalg.norm(pw.k) * np.real(self.n1)
                
                yz_term = yHat * 1j * np.sqrt(self.sin_theta_i**2 * np.real(n0)**2 / np.real(n1)**2 - 1) - zHat * np.real(n0) / np.real(n1) * self.sin_theta_i
                Et = tp * Ei_p * yz_term + xHat * ts * Ei_s
        ...
        return [planewave(kr, Er*np.exp(phase_r * 1j), self.tir, False, self.sin_theta_i), planewave(self.kt, Et*np.exp(phase_t * 1j), self.tir, True, self.sin_theta_i)]
    ...
    def evaluate(self, X, Y, Z, pw):
        ...
        if(self.tir == False):
            ...
            #Todo: for TIF, transmitted wave k is set to 0 right now. Mask cannot be calculated in this case
            t =  -(np.dot(self.p, self.N) - ( X * self.N[0] + Y * self.N[1] + Z * self.N[2])) / np.dot( Wt.k, self.N)
            decay = np.exp( - np.imag(self.n1) * t )
        else:
            decay = np.exp(-self.kt[2] * Z * np.sqrt(self.sin_theta_i**2 * n0**2 / np.real(n1)**2 - 1))
...
n0 = 1.0
n1 = 0.8                                   #n = nt / ni (n0 is the source material(incidental), nt is the material after the interface(transmitted))
...
Ei = planewave(k, E, False, False, 0)
